...
import { fetchTicketListData } from 'src/app/store/Ticket/ticket_action';
...
export class DashboardComponent implements OnInit {
  ...
  constructor(public toastService: ToastService, private store: Store<{ data: RootReducerState }>,) {
    var date = new Date();
 ...
 */
  ...
  */
  private fetchData() {
    ...
    this.store.select(selectTicketData).pipe(take(1)).subscribe((data) => {
      if (!data || data.length === 0) {
        this.store.dispatch(fetchTicketListData());
      } else {
        console.log("Ticket Data (From Store):", data);
      }
    });

    this.store.select(selectTicketLoading).subscribe((loading) => {
      if (loading === false) {
    ...
    this.store.select(selectTicketData).subscribe((data) => {
      this.BestSelling = data.map((item: any) => ({
        image: "assets/images/companies/bugs.png",
        pName: item.title,
        date: item.create,
        price: item.due.substring(0, 10),
        orders: item.client,
        stock: item.priority,
        amount: item.id,
      }));
    ...
    this._apiCallService.GetCall('ApplicationUser/GetUsers').subscribe((response: ResponseVM) => {
      if (response.responseCode == ResponseCode.Success) {
        ...
        if (response.data && response.data.length > 0) {
          this.TopSelling = response.data.map((item: any) => ({
            image: item.profileImageUrl,
            pName: item.firstName + ' ' + item.lastName,
            subtitle: item.userName,
            type: item.userEmail.substring(0, 15) + '...',
            // stock: item.phoneNumber,
            amount: item.gender,
            percentage: '90'
          }));
        } else {
