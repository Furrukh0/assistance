...
export class restApiService {
  constructor(private http: HttpClient, private _apiCallSerice: ApiCallService) { }
  ...
  */
  // Get
  getTicketData(payload: any): Observable<any> {
    // var headerToken = { 'Authorization': `Bearer ` + sessionStorage.getItem('token') };
    // return this.http.get(GlobalComponent.API_URL + 'apps/ticket', { headers: headerToken, responseType: 'text' });
    return this._apiCallSerice.PostCall(payload, 'UserTicket/GetTickets');
  }
  ...
  postTicketData(ticket: any): Observable<any> {
    ...
    return this._apiCallSerice.PostCall(ticket, 'UserTicket/CreateOrUpdateTicket');
  }
  ...
  deleteTicket(id: any): Observable<any> {
    ...
    const ID = Number(id);
    return this._apiCallSerice.DeleteCall(`UserTicket/DeleteTicket/${ID}`)
  }

  // Get All Users
  getUsersList(): Observable<any> {
    return this._apiCallSerice.GetCall('ApplicationUser/get-users-list');
  }
